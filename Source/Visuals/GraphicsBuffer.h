...
protected:
	...
	const Visuals::GraphicsMode&  	m_GraphicsMode;     // the graphic mode definition
	...
	bool                            m_Drawing;          // flag: Indicates we are drawing pixels; don't render immediately
						  ...
						  GraphicsBuffer(unsigned int width, unsigned int height, const Visuals::GraphicsMode& mode);
...
std::vector<unsigned char>          // Get the native byte data for the buffer
                          GetNative(ImageTypes type) const;
...
//
//public:
//                          CharacterMapGraphicsBuffer(unsigned int width, unsigned int height, const Agdx::GraphicsMode& mode);
//
//    void                  Render() const;
